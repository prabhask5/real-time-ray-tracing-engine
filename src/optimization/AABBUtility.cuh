...
__device__ __forceinline__ CudaAABB operator+(const CudaAABB &box,
  ...
  return CudaAABB(box.x + offset.x, box.y + offset.y, box.z + offset.z);
}
...
cuda_bbox_compare(const CudaAABB &a, const CudaAABB &b, int axis_index) {
  const CudaInterval &a_axis = a.get_axis_interval(axis_index);
  const CudaInterval &b_axis = b.get_axis_interval(axis_index);
