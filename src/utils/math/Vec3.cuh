...
public:
  ...
  __device__ Vec3 &operator/=(double t) { return *this *= 1.0 / t; }

  // Complex getter methods

  __device__ double length() const { return sqrt(length_squared()); }
  ...
  __device__ bool near_zero() const {
    double s = 1e-8;
    return (fabs(m_coordinates[0]) < s) && (fabs(m_coordinates[1]) < s) &&
  ...
  __device__ static Vec3 random(curandState *state) {
    return Vec3(random_double(state), random_double(state),
                random_double(state));
  }

  __device__ static Vec3 random(curandState *state, double min, double max) {
    return Vec3(random_double(state, min, max), random_double(state, min, max),
