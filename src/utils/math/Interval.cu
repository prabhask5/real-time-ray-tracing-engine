...
__global__ void clamp_values_kernel(double *values, const Interval *intervals,
                                    int count) {
...
void batch_clamp_values(double *d_values, const Interval *d_intervals,
                        int count) {
  int blockSize = 256;
...
__global__ void intersect_intervals_kernel(const Interval *intervals1,
...
void batch_intersect_intervals(const Interval *d_intervals1,
                               const Interval *d_intervals2, Interval *d_result,
                               int count) {
  int blockSize = 256;
  int numBlocks = (count + blockSize - 1) / blockSize;
