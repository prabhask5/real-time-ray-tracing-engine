...
public:
  ...
  __device__ PerlinNoise(Vec3 *rand_vec, int *perm_x, int *perm_y, int *perm_z)
      : m_rand_vec(rand_vec), m_perm_x(perm_x), m_perm_y(perm_y),
        m_perm_z(perm_z) {}
...
private:
  ...
  __device__ double perlin_interp(const Vec3 c[2][2][2], double u, double v,
                                  double w) const {
    double uu = u * u * (3 - 2 * u);
    double vv = v * v * (3 - 2 * v);
    double ww = w * w * (3 - 2 * w);
...
inline void cuda_allocate_perlin_noise(Vec3 **d_rand_vec, int **d_perm_x,
                                       int **d_perm_y, int **d_perm_z) {
  ...
  // Copy to device.
  cudaMemcpy(*d_rand_vec, host_rand_vec, PERLIN_POINT_COUNT * sizeof(Vec3),
             cudaMemcpyHostToDevice);
  cudaMemcpy(*d_perm_x, host_perm_x, PERLIN_POINT_COUNT * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(*d_perm_y, host_perm_y, PERLIN_POINT_COUNT * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(*d_perm_z, host_perm_z, PERLIN_POINT_COUNT * sizeof(int),
             cudaMemcpyHostToDevice);
}

// Host function to free Perlin noise device memory.
inline void cuda_free_perlin_noise(Vec3 *d_rand_vec, int *d_perm_x,
                                   int *d_perm_y, int *d_perm_z) {
  if (d_rand_vec)
    cudaFree(d_rand_vec);
  if (d_perm_x)
